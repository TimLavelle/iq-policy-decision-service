...
const KNOWN_RULES = [
  ...
  'disruption-autorebook-eligibility.json',
  'disruption-ssr-escalation.json',
  'disruption-connection-thresholds.json',
  'disruption-queue-eligibility.json',
  'disruption-compensation.json',
  'disruption-type-classification.json',
  'disruption-award-handling.json',
  // Baggage
...
const RULE_META: Record<string, { displayName: string; description: string; inputs: string[]; outputs: string[]; decisionEndpoint: string; group: string }> = {
  ...
  'disruption-autorebook-eligibility.json': {
    group: 'Disruption',
    displayName: 'Auto-Rebook Eligibility',
    description: 'Determines whether a passenger qualifies for AI Auto-Rebook or falls through to agent review, based on tier, booking type, itinerary complexity, and journey time increase.',
    inputs: ['customerTier', 'connectionAtRisk', 'cabinClass', 'isAwardBooking', 'segmentCount', 'journeyTimeIncreaseMins'],
    outputs: ['eligible', 'reason', 'policyRef'],
    decisionEndpoint: '/v1/decisions/autorebook-eligibility',
  },
  'disruption-ssr-escalation.json': {
    group: 'Disruption',
    displayName: 'SSR Escalation Check',
    description: 'Determines whether a passenger\'s special service requirements (WCHR, UMNR, DEAF, etc.) or interline operating carrier require CC escalation during disruption.',
    inputs: ['ssrCodes', 'operatingCarrier'],
    outputs: ['requiresCcEscalation', 'escalationReason', 'policyRef'],
    decisionEndpoint: '/v1/decisions/escalation-check',
  },
  'disruption-connection-thresholds.json': {
    group: 'Disruption',
    displayName: 'Connection Minimum Time',
    description: 'Returns minimum and recommended connection times by route type (domestic/international), with airport-specific overrides for SYD, MEL, and BNE.',
    inputs: ['routeType', 'connectionAirport'],
    outputs: ['minimumConnectionMinutes', 'recommendedConnectionMinutes', 'policyRef'],
    decisionEndpoint: '/v1/decisions/connection-mctime',
  },
  'disruption-queue-eligibility.json': {
    group: 'Disruption',
    displayName: 'Disruption Queue Eligibility',
    description: 'Determines whether a passenger appears in the disruption management queue or is directed to self-service, based on tier, cabin, connection risk, and SSR codes.',
    inputs: ['customerTier', 'cabinClass', 'connectionAtRisk', 'ssrCodes'],
    outputs: ['queueEligible', 'reason', 'policyRef'],
    decisionEndpoint: '/v1/decisions/queue-eligibility',
  },
  'disruption-compensation.json': {
    group: 'Disruption',
    displayName: 'Disruption Compensation',
    description: 'Calculates Travel Bank credit (AUD) and QFF points compensation by disruption type and customer tier. Returns zero and ineligible for delays under 2 hours.',
    inputs: ['disruptionType', 'customerTier', 'cabinClass'],
    outputs: ['travelBankCreditAud', 'pointsCredit', 'eligible', 'policyRef'],
    decisionEndpoint: '/v1/decisions/compensation-amount',
  },
  'disruption-type-classification.json': {
    group: 'Disruption',
    displayName: 'Disruption Type Classification',
    description: 'Maps delay duration in minutes and cancellation flag to a structured disruption type label and severity level (low/medium/high/critical).',
    inputs: ['delayMinutes', 'cancelled'],
    outputs: ['disruptionType', 'severity', 'policyRef'],
    decisionEndpoint: '/v1/decisions/classify-disruption',
  },
  'disruption-award-handling.json': {
    group: 'Disruption',
    displayName: 'Award Booking Disruption Handling',
    description: 'Determines the correct action for award (points redemption) bookings during disruption — auto-rebook, agent review, or standard rebook — and whether points should be restored.',
    inputs: ['isAwardBooking', 'hasAwardInventoryOnAlternative', 'customerTier'],
    outputs: ['action', 'restorePoints', 'reason', 'policyRef'],
    decisionEndpoint: '/v1/decisions/award-handling',
  },
  // ── Baggage ───────────────────────────────────────────────────────────────
  'baggage-claim-threshold.json': {
    group: 'Baggage',
